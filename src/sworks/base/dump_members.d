module sworks.base.dump_members;


////////////////////////////////////////////////////////////////////////////////
/// Dのオブジェクト -> メンバ名とその内容を表す文字列 デバグ用
string dump_members(THIS)(THIS t, uint max_indent = 12, uint max_array = 64
                           , uint indent = 0)
{
    ...
    import std.traits : isCallable, isSomeString, isAssociativeArray;
    enum TAB_SIZE = 4;
                ...
                if (indent < max_indent)
                {
                    foreach (one ; __traits(derivedMembers, THIS))
                    {
                        static if (__traits(compiles
                            , __traits(getMember, t, one).offsetof))
                        {
                            result.put(take(repeat(' ')
                                            , indent * TAB_SIZE));
                            result.put(one);
                            result.put(" = ");
                            result.put(dump_members(
                                  __traits(getMember, t, one), max_indent
                                , max_array, indent));
                            result.put(newline);
                        }
                    }
            ...
            if (indent < max_indent)
            {
                foreach (one ; __traits(derivedMembers, THIS))
                {
                                       ...
                                       , __traits(getMember, t, one).offsetof))
                    {
                        ...
                        result.put(one);
                        result.put(" = ");
                        result.put(dump_members(__traits(getMember, t, one)
                                                , max_indent, max_array
                                                , indent));
                        result.put(newline);
                    }
                }
    ...
    {
        if (null !is t)
            return (*t).dump_members(max_indent, max_array, indent);
        else return "null";
    }
