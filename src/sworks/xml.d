...
class XMLText : AXML
{
    ...
    override string toString() const { return text; }
    //
    @property @trusted pure nothrow
    override XMLText dup() const
...
class XML : AXML
{
    ...
    /// 登場順に要素を巡回する。
    @trusted
    override int opApply(scope int delegate(AXML) dg)
    {
        auto result = dg(this);
    ...
    /// ditto
    @trusted
    override int opApply(scope int delegate(in AXML) dg) const
    {
        auto result = dg(this);
        if (result) return result;
        for (size_t i = 0; i < children.length; ++i)
    ...
    //
    @property @trusted pure nothrow
    private string getOpenTagString()
...
///
class IntValue : AttributeValue
{
    ///
    int value;
    ///
    @trusted @nogc pure nothrow
    this(in int v) { value = v; }
...
///
class UintValue : AttributeValue
{
    ///
    uint value;
    ///
    @trusted @nogc pure nothrow
    this(in uint v) { value = v; }
...
class XMLParserInfo
{
    ...
    **/
    @trusted
    AttributeValue attrValueParser(TICache!char buf, in Ns n)
     ...
     */
    @trusted
    protected void parseStyle(TICache!char buf)
    {
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     */
    @trusted
    protected AttributeValue styleValueParser(TICache!char buf, in Ns)
    { return buf.munchStringValue; }
}
...
**/
@trusted
private AXML parseXML(XML_PARSER_PROPERTY P = XML_PARSER_PROPERTY.STANDARD)
...
@trusted
private AXML munchTag(bool LC)(TICache!char buf, ref XMLParserInfo info)
{
    assert(info !is null);
    assert('<' == buf.front);
...
@trusted
private Ns munchStyleKey(TICache!char buf)
{
...
@trusted
private Ns munchNs(bool LC)(TICache!char buf, string[string] ns,
                            lazy string def = "")
{
...
//
@trusted
private Attribute toCSSAttr(TICache!char buf, XMLParserInfo info)
{
    import std.string : strip;
    buf.stripLeftCSSWhite;
